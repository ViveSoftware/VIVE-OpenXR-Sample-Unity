    ...
    public abstract partial class UnityXRModuleBase : VRModule.ModuleBase
    {
...
#if !VIU_WAVEVR_HAND_TRACKING
        private List<XRHandSubsystem> s_XRHandSubsystems = new List<XRHandSubsystem>();
        ...
        public sealed override void BeforeRenderUpdate()
        {
            ...
            foreach (var device in connectedDevices)
            {
                ...
                // update device Poses
                currState.isPoseValid = GetDeviceFeatureValueOrDefault(device, CommonUsages.isTracked) && !CheckIsNaN(currState);

                if (prevState.isPoseValid && !currState.isPoseValid)
                {
                    if (CheckIsNaN(currState))
                    {
                        Debug.Log("[VIU][UnityXRModule] NaN value is found");
                    }
                }

#if !VIU_WAVEVR_HAND_TRACKING
                if ((device.characteristics & InputDeviceCharacteristics.HandTracking) > 0u)
                {
                    ...
                    if (handSubsystem != null && handSubsystem.running)
                    {
                        if (handSubsystem.rightHand.isTracked && (device.characteristics & InputDeviceCharacteristics.Right) > 0u)
                        {
                            UpdateFingerJoints(handSubsystem.rightHand, currState, HandJointName.ThumbTrapezium, HandJointName.ThumbTip);
                            UpdateFingerJoints(handSubsystem.rightHand, currState, HandJointName.IndexMetacarpal, HandJointName.IndexTip);
                            UpdateFingerJoints(handSubsystem.rightHand, currState, HandJointName.MiddleMetacarpal, HandJointName.MiddleTip);
                            UpdateFingerJoints(handSubsystem.rightHand, currState, HandJointName.RingMetacarpal, HandJointName.RingTip);
                            UpdateFingerJoints(handSubsystem.rightHand, currState, HandJointName.PinkyMetacarpal, HandJointName.PinkyTip);

                            XRHandJoint xrHandJoint = handSubsystem.rightHand.GetJoint(ConvertToXRHandJointID(HandJointName.Palm));

                            if (xrHandJoint.TryGetPose(out Pose palm))
                            {
                                var p = default(RigidPose);
                                p.pos = palm.position;
                                p.rot = palm.rotation;
                                currState.handJoints[HandJointName.Palm] = new JointPose(p);
                            }

                            xrHandJoint = handSubsystem.rightHand.GetJoint(ConvertToXRHandJointID(HandJointName.Wrist));

                            if (xrHandJoint.TryGetPose(out Pose wrist))
                            {
                                var p = default(RigidPose);
                                p.pos = wrist.position;
                                p.rot = wrist.rotation;
                                currState.handJoints[HandJointName.Wrist] = new JointPose(p);
                            }
                        }

                        if (handSubsystem.leftHand.isTracked && (device.characteristics & InputDeviceCharacteristics.Left) > 0u)
                        {
                            UpdateFingerJoints(handSubsystem.leftHand, currState, HandJointName.ThumbMetacarpal, HandJointName.ThumbTip);
                            UpdateFingerJoints(handSubsystem.leftHand, currState, HandJointName.IndexMetacarpal, HandJointName.IndexTip);
                            UpdateFingerJoints(handSubsystem.leftHand, currState, HandJointName.MiddleMetacarpal, HandJointName.MiddleTip);
                            UpdateFingerJoints(handSubsystem.leftHand, currState, HandJointName.RingMetacarpal, HandJointName.RingTip);
                            UpdateFingerJoints(handSubsystem.leftHand, currState, HandJointName.PinkyMetacarpal, HandJointName.PinkyTip);

                            XRHandJoint xrHandJoint = handSubsystem.leftHand.GetJoint(ConvertToXRHandJointID(HandJointName.Palm));
                            if (xrHandJoint.TryGetPose(out Pose palm))
                            {
                                var p = default(RigidPose);
                                p.pos = palm.position;
                            ...
                            xrHandJoint = handSubsystem.leftHand.GetJoint(ConvertToXRHandJointID(HandJointName.Wrist));
                            if (xrHandJoint.TryGetPose(out Pose wrist))
                            {
                                var p = default(RigidPose);
                                p.pos = wrist.position;
                ...
                if (currState.isPoseValid && (device.characteristics & InputDeviceCharacteristics.Right) > 0u)
                {
                    uxrRightIndex = deviceIndex;
                }
                else if (currState.isPoseValid && (device.characteristics & InputDeviceCharacteristics.Left) > 0u)
                {
        ...
        private bool CheckIsNaN(IVRModuleDeviceStateRW currState)
        {
            var pos = currState.position;
            if (float.IsNaN(pos.x) || float.IsNaN(pos.y) || float.IsNaN(pos.z)) return true;
            var rot = currState.rotation;
            if (float.IsNaN(rot.w) || float.IsNaN(rot.x)
                || float.IsNaN(rot.y) || float.IsNaN(rot.z)) return true;
            var vel = currState.velocity;
            if (float.IsNaN(vel.x) || float.IsNaN(vel.y) || float.IsNaN(vel.z)) return true;
...
#if !VIU_WAVEVR_HAND_TRACKING
        private void UpdateFingerJoints(XRHand hand, IVRModuleDeviceStateRW state, HandJointName startJoint, HandJointName endJoint)
        {
            foreach (var index in EnumArrayBase<HandJointName>.StaticEnumsFrom(startJoint, endJoint))
            {
                XRHandJoint xrHandJoint = hand.GetJoint(ConvertToXRHandJointID(index));
                if (xrHandJoint.TryGetPose(out Pose pose))
                {
                    var p = default(RigidPose);
                    p.pos = pose.position;
                    p.rot = pose.rotation;
                    state.handJoints[index] = new JointPose(p);
                }
            }
        }

        private static XRHandJointID ConvertToXRHandJointID(HandJointName jointType)
        {
