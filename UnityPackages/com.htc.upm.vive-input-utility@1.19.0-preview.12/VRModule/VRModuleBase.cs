...
namespace HTC.UnityPlugin.VRModuleManagement
{
    public partial class VRModule : SingletonBehaviour<VRModule>
    {
        public abstract class ModuleBase
        {
            ...
            private static readonly Regex s_viveTagRgx = new Regex("^.*tag.*$", REGEX_OPTIONS);
            ...
            private static WVRCtrlProfile[] s_wvrCtrlProfiles = new WVRCtrlProfile[]
            {
                ...
                // Vive Hand OpenXR Left
                new WVRCtrlProfile { reg = new Regex("^.*hand.+left.*$", REGEX_OPTIONS), model = VRModuleDeviceModel.WaveTrackedHandLeft, input2D = VRModuleInput2DType.None },
                // Vive Hand OpenXR Right
                new WVRCtrlProfile { reg = new Regex("^.*hand.+right.*$", REGEX_OPTIONS), model = VRModuleDeviceModel.WaveTrackedHandRight, input2D = VRModuleInput2DType.None },
            };
                ...
                if (s_viveRgx.IsMatch(deviceState.modelNumber) || s_viveRgx.IsMatch(deviceState.renderModelName))
                {
                    switch (deviceState.deviceClass)
                    {
                        ...
                        case VRModuleDeviceClass.GenericTracker:
                            ...
                            else if (s_viveTagRgx.IsMatch(deviceState.renderModelName))
                            {
                                deviceState.deviceModel = VRModuleDeviceModel.ViveTrackingTag;
                            }
                            else if (s_ver3Rgx.IsMatch(deviceState.modelNumber))
                            {
                                deviceState.deviceModel = VRModuleDeviceModel.ViveTracker3;
                            }
                            else if (s_wvrCtrlProfiles[5].reg.IsMatch(deviceState.serialNumber))
                            {
                                deviceState.deviceModel = s_wvrCtrlProfiles[5].model;
                                deviceState.input2DType = s_wvrCtrlProfiles[5].input2D;
                            }
                            else
                            {
