...
public class PortalSceneManager : Singleton<PortalSceneManager>
{
    ...
    private void CreatPortalBall(int portalType, Vector3 spawnPoint, Vector3 flyDirection, float radius)
    {
        ...
        {
            Debug.Log($"[Debug] Hit object: {hit.collider.gameObject.name}, Position: {hit.point}");
        }
        ...
        {
            Debug.LogError("[PortalSceneManager][CreatPortalBall] No valid PlaneController found.");
            tryCreatingPortal = false;
            return;
        }
        ...
        for (int i = 0; i < 4; i++)
        {
            ...
            // Ensure planeController is still valid
            if (planeController == null)
            {
                Debug.LogError("[PortalSceneManager][CreatPortalBall] planeController became null during loop.");
                tryCreatingPortal = false;
                return;
            }
            ...
            if (Vector3.Distance(toSide, destination) < radiusLength)
            {
                ...
                else
                {
                    ...
                    if (SceneComponentManager.Instance != null)
                    {
                        for (int j = 0; j < SceneComponentManager.Instance.Planes.Count; j++)
                        {
                            if (SceneComponentManager.Instance.Planes[j].Type == "wall")
                            {
                                ...
                                {
                                    int kPlus = k < 3 ? k + 1 : 0;
                                    if ((SceneComponentManager.Instance.Planes[j].Points[k] == planeController.Data.Points[i] &&
                                         SceneComponentManager.Instance.Planes[j].Points[kPlus] == planeController.Data.Points[iPlus]) ||
                                        (SceneComponentManager.Instance.Planes[j].Points[k] == planeController.Data.Points[iPlus] &&
                                         SceneComponentManager.Instance.Planes[j].Points[kPlus] == planeController.Data.Points[i]))
                                    {
                    ...
                    {
                        Debug.Log("SceneComponentManager.Instance is null.");
                        return;
                    }
                    ...
                    Vector3 fn = firstPortalBoundary - CommonFormula.PointFromPointToLine(
                        new Vector3(spawnPoint.x, firstPortalBoundary.y, spawnPoint.z),
                        new Vector3(toSide.x, firstPortalBoundary.y, toSide.z),
                        firstPortalBoundary);
