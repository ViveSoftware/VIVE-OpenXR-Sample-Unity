...
namespace VIVE.OpenXR.StarterSample.HandTracking
{
                ...
                bool debugDrawJoints)
            {
                    ...
                    ++fingerIndex)
                {
                    ...
                    for (var jointIndex = fingerId.GetFrontJointID().ToIndex();
                        ...
                        ++jointIndex)
                    {
                        UpdateJoint(debugDrawJoints, hand.GetJoint(XRHandJointIDUtility.FromIndex(jointIndex)), ref parentPose, ref parentIndex);
                    }
                }
            }
